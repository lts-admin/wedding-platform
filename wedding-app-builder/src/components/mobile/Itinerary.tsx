...
const Itinerary: React.FC<ItineraryProps> = ({ form, setForm, goNext, goBack }) => {
    ...
    const renderEventInputs = (section: "weddingEvents" | "brideEvents" | "groomEvents", label: string) => (
        <div className="space-y-4">
            ...
            {form[section].map((event, index) => (
                <div key={index} className="border p-4 rounded-md space-y-2">
                    ...
                    <Input
                        ...
                        className="[color-scheme:dark] bg-[#1A1A1A] mb-2 font-bold"
                        disabled={isSubmitted}
                    />
                    <Input
                        placeholder="Time"
            ...
            <Button type="button" className="bg-pink-500 text-black" onClick={() => addEvent(section)} disabled={isSubmitted}>+ Add {label} Event</Button>
        </div>
    );

    return (
        <div className="max-w-2xl space-y-6">
            <h2 className="text-2xl font-semibold text-pink-500">Wedding Itinerary</h2>
